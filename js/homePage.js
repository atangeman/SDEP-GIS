...
function dojoInit() {
    ...
    if ((userAgent.indexOf("Android") >= 0 && userAgent.indexOf("Mobile") >= 0) || userAgent.indexOf("iPhone") >= 0) {
        ...
        isBrowser = true;
        dojo.byId('report').style.display = 'none';
...
/*
    ...
    */
    ...
    dojo.connect(map, "onClick", function (evt) {
        ...
        //Hackathon update - disable map click submissions by default - atangeman20151005
...
/*
    var localizedHazardsLayer = new esri.layers.FeatureLayer(operationalLayers.LocalizedHazardsLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(localizedHazardsLayer);

    var trafficControlLayer = new esri.layers.FeatureLayer(operationalLayers.TrafficControlLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(trafficControlLayer);

    var evacuationCentersLayer = new esri.layers.FeatureLayer(operationalLayers.EvacuationCentersLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(evacuationCentersLayer);

    var recoveryCentersLayer = new esri.layers.FeatureLayer(operationalLayers.RecoveryCentersLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(recoveryCentersLayer);

    var evacuationRoutesLayer = new esri.layers.FeatureLayer(operationalLayers.EvacuationRoutesLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(evacuationRoutesLayer);

    var evacuationNotificationAreasLayer = new esri.layers.FeatureLayer(operationalLayers.EvacuationNotificationAreasLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(evacuationNotificationAreasLayer);

    var firePerimiterLayer = new esri.layers.FeatureLayer(operationalLayers.FirePerimiterLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
    map.addLayer(firePerimiterLayer);

    var fireStationsLayer = new esri.layers.FeatureLayer(operationalLayers.FireStationsLayer, {
        mode: esri.layers.FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"]
    });
