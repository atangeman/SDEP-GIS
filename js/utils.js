...
function SubmitIssueDetails() {
    ...
    var idx = dojo.byId('txtSeverity').selectedIndex;
    serviceRequestAttributes[serviceRequestFields.RequestTypeFieldName] = dojo.byId("txtSelectedRequest").value;
    serviceRequestAttributes[serviceRequestFields.RequestSeverityLevel] = dojo.byId('txtSeverity').options[idx].value;
        ...
        if (addResults[0].success) {
            ...
            map.getLayer(serviceRequestLayerId).applyEdits(null, [requestGraphic], null, function () {
                ...
                if (dojo.byId('txtFileName').value != "") {
                  //File upload
                    map.getLayer(serviceRequestLayerId).addAttachment(addResults[0].objectId, formData, function (sucess) {
...
function ShowAddressSearchView() {
  dojo.byId("divAddressPlaceHolder").style.display = "block";
...
function ShowRequestSearchView() {
    ...
    */
    dojo.byId("divAddressPlaceHolder").style.display = "none";
    dojo.byId("txtAddress").value = dojo.byId("txtAddress").getAttribute("defaultRequestName");
    lastSearchString = dojo.byId("txtAddress").value.trim();
    dojo.empty(dojo.byId('tblAddressResults'));
    RemoveScrollBar(dojo.byId('divAddressScrollContainer'));
...
function ShowToc() {
  dojo.byId('divTocContainer').style.display = 'block';
}
var video = null;
var canvas = null;
...
function takePic() {
  ...
  var videoSelect = dojo.byId('videoSource');
  videoSelect.options.length = 0;
  video = dojo.byId("videoElement");
  video.style.width = infoPopupWidth + "px";
  video.style.height = infoPopupHeight + "px";
...
function snap() {
      ...
      formData.append("attachment", blob, imageName+"."+imageFormat);
      dojo.byId('txtFileName').value = "attachment";
      CloseUploadDialog();
		}
